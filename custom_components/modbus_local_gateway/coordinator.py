...
class ModbusCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hass: HomeAssistant, gateway_device: dr.DeviceEntry, client: AsyncModbusTcpClientGateway, gateway: str, update_interval: int = 30) -> None:
    ...
    async def _async_refresh(self, log_failures: bool = False,
    ...
    async def _async_update_data(self):
        ...
        entities: list[ModbusContext] = sorted(self.async_contexts(), key=lambda x:x.slave_id)
    ...
    async def _update_device(self, entities: list[ModbusContext]) -> dict[str, Any]:
        ...
        resp: dict[str, ModbusResponse] = await self.client.update_slave(entities, max_read_size=self._max_read_size)
        data: dict[str, Any] = {}
    ...
    def translate_data(self, ctx: ModbusContext, response: ModbusResponse) -> str | int | None:
        """returns the value of the registers"""
        try:
            if ctx.desc.string:
                value: str = self.client.convert_from_registers(response.registers,
                                                                data_type=self.client.DATATYPE.STRING).split("\0")[0]
                return value
            if ctx.desc.float:

                value: float =  self.client.convert_from_registers(response.registers,
            ...
            num = self.client.convert_from_registers(response.registers,
                                                     data_type=self.client.DATATYPE.INT32 if ctx.desc.register_count == 2 else self.client.DATATYPE.INT16)

            return num * ctx.desc.register_multiplier
        except Exception: # pylint: disable=broad-exception-caught
            _LOGGER.debug("Data not available for key: %s (%d)", ctx.desc.key, ctx.slave_id)
