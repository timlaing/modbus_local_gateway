...
async def async_setup_entry(
    hass: HomeAssistant, entry: config_entries.ConfigEntry) -> bool:
    ...
    if gateway_key not in hass.data[DOMAIN]:
        client: AsyncModbusTcpClientGateway = await AsyncModbusTcpClientGateway.async_get_client_connection(hass=hass,
                                                                                                            data=entry.data)
        if client is not None:
            hass.data[DOMAIN][get_gateway_key(entry=entry)] = ModbusCoordinator(hass=hass,
                                                                                gateway_device=device,
                                                                                client=client,
                                                                                gateway=gateway_key,
                                                                                update_interval=entry.options.get(OPTIONS_REFRESH,
                                                                                                                  OPTIONS_DEFAULT_REFRESH))
        else:
