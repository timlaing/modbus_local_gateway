...
- Full default-stage set: `prek run --all-files` — or `prek run --files <paths>` for a subset. Do not use `pre-commit` directly for regular runs. mypy and pylint are manual-stage hooks only.
